 ...
 */
export default function productPageScript() {
  ...
  if (
    bd.contains("productlist-list-page") ||
    bd.contains("productlist-search-page")
  ) {
    let sortOptions = document.getElementById("Sorting").childNodes;
    for (let i = 0; i < sortOptions.length; i += 1) {
      if (sortOptions[i].innerHTML == "U_SalesQty - ascending")
        sortOptions[i].remove();
    }
  }
  ...
  if (bd.contains("product-details-page")) {
    ...
    if (data.salesUomName === "Sales Pack") {
      ...
      if (document.getElementsByClassName("lbl-uom").length > 0) {
        ...
        let currencyStr = document
          .querySelectorAll('[itemprop="priceCurrency"]')[0]
          .getAttribute("content");
        let currency = currencyStr === "EUR" ? "€" : "£";
        let price = document
          .querySelectorAll('[itemprop="price"]')[0]
          .getAttribute("content");
        ...
        function addSalesPackEventListeners() {
          let qtyBox = document.querySelector(
            ".row-quantity .tbx-quantity input"
          );
          qtyBox.addEventListener("keyup", updateSalesPackInfo);

          let qtyPlus = document.querySelectorAll(
            ".row-quantity .tbx-quantity a"
          )[0];
          qtyPlus.addEventListener("click", updateSalesPackInfo);

          let qtyMinus = document.querySelectorAll(
            ".row-quantity .tbx-quantity a"
          )[1];
        ...
        function updateSalesPackInfo() {
          ...
          label.innerHTML = `Sales Packs x ${qty} = ${
            data.itemsPerSalesUnit * qty
          } Units (${currency}${newPrice})`;
        }
      }
    }
  }
